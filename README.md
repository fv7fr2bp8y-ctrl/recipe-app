...
> **Защо така?** По-рано `recipes.json` се теглеше при всеки деплой от Google Drive през Google Apps Script. Това водеше до проблеми с дублирани файлове в Drive и счупени снимки, защото не беше ясно кой файл се чете. Сега repo-то е източникът — стабилно и проследимо през git история.
...
- Може да добавя/редактира/изтрива рецепти **в текущата сесия** (записват се локално и в Drive)
- ⚠️ Admin промените **не се публикуват автоматично** на сайта. За да станат живи, отрази ги в `public/recipes.json` и push-ни в `main`.
