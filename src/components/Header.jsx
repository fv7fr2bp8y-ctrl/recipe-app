...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-orange-100 bg-[#fffaf2]/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          <img src="/tastemaster-mark.svg" alt="" className="h-10 w-10 shrink-0 sm:h-12 sm:w-12" />
          <div className="min-w-0">
            ...
            <p className="mt-1 hidden text-[10px] uppercase tracking-[0.24em] text-orange-700 sm:block">{messages.tagline}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative" ref={languageMenu}>
            <button
              ...
              className="flex h-10 items-center gap-1 border border-orange-200 bg-white px-2 text-xs font-semibold text-stone-700 hover:border-orange-400 sm:gap-1.5 sm:px-2.5"
              ...
              <div className="absolute right-0 top-12 z-50 min-w-44 border border-orange-200 bg-white p-1.5 shadow-xl">
                {LANGUAGES.map((item) => (
                  <button
                    ...
                    className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm ${language === item.code ? 'bg-orange-50 text-orange-800' : 'text-stone-700 hover:bg-stone-50'}`}
                  >
          ...
          <button
            type="button"
            onClick={onShare}
            className="flex h-10 w-10 items-center justify-center border border-orange-200 bg-white text-orange-800 hover:border-orange-400"
